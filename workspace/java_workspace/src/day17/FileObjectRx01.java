...
public class FileObjectRx01 {

	public static void main(String[] args) {
		// 파일이 가지고 있는 정보를 출력.
		// D:\workspace\java_workspace
		
		File f = new File("D:\\workspace\\java_workspace\\out.txt");
		String fileName = f.getName();
		System.out.println(fileName); // 경로를 제외한 파일이름
		System.out.println(f.getPath()); // 경로+이름
		System.out.println(f.getAbsolutePath()); //경로+이름(절대경로)
		System.out.println(f.getParent()); //경로만
		System.out.println(File.separator); // 파일구분자
		
		//시스템에서 직접 알아낸 사용자파일로 경로는 분리
