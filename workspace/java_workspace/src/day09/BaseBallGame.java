...
public class BaseBallGame {
	...
	//랜덤번호 생성 : 외부 매개변수로 범위를 설정 20~30 ->cnt:10 start:20
...
//	}
	...
	public static boolean randomArray(int arr[], int max, int min) {
		if(arr==null) { //배열의 값이 없을 경우
			return false;
		}
		if((max-min+1)<arr.length) { //랜덤수의 개수가 배열의 length보다 작다면 
			return false;
		}
		int i =0;
		while(i<arr.length) {
