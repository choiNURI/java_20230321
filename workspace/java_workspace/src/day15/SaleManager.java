...
public class SaleManager {
	...
	public void orderPick(Scanner scan) {
		...
		for(int i=0;i<product.size();i++) {
			if(product.get(i).getMenu().equals(orderManu)) {
				index = i;
				int price = product.get(i).getPrice();
				Sale s = new Sale(orderManu,orderCount*price);
				order.add(s);
			}
		}
		if(index==-1) {
			System.out.println("찾는 메뉴가 없어요");
		}
	}
	...
	public void printMenu() {
		...
		System.out.println(" 1.제품추가|2.리스트확인");
		System.out.println(" 3.제품주문|4.주문내역");
